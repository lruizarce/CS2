...
class LinkedList(object):
    ...
    def pop_back(self):
        if self.empty():
            raise RuntimeError("Empty linked list")
        elif (self.front.next_node == None):
            tmp = self.front
            self.front = None
            self.back = None
            return tmp.value
            ...
            newLast = self.front
            while(newLast.next_node.next_node != None):
                newLast = newLast.next_node
            self.back = newLast
            tmp = newLast.next_node
            self.back.next_node = None
            return tmp.value
    ...
    def __str__(self):
        str = ""
        if not self.empty():
            while(self.back is not self.front):
                str = str + self.back.value + ", "
                self.pop_back()
            str = str + self.front.value
            self.pop_back()
            return str
